...
1. Install `nebula` on all the nodes.
```
# sudo pacman -S nebula
```

2. Choose a node that will act as a certificate authority. Create a directory,
...
3. Choose one or more nodes to serve as lighthouses. They must have a static IP
address.

  In my case, I decided to designate `draco` as a lighthouse, since it was the
only node that faced the open internet.

4. For each node, generate a certificate pair signed by the certificate
authority:
```
...
```
  ...
  This will create the files `NODE_NAME.key` and `NODE_NAME.crt` for each
node.

5. Copy each `NODE_NAME.key` and `NODE_NAME.crt` onto their respective hosts,
as well as the `ca.crt` file generated by the certificate authority. Place them
in the folder `/etc/nebula/`.

6. On the lighthouse node, create the configuration file at `/etc/nebula/config.yml`
...
static_host_map:
  "10.42.0.1": ["YOUR_STATIC_IP_ADDRESS:4242"]
...
7. On regular nodes, the file will be mostly the same, except for the following
changes:
```
lighthouse:
  am_lighthouse: false
...
8. You can tinker with these settings and figure out what works best for you.

9. On your lighthouse node, install `dnsmasq`, and assign static IP addresses
to each of your nodes by adding the following lines to `/etc/dnsmasq.conf`:
```
...
10. Enable the `dnsmasq` daemon on the lighthouse node.

11. On each regular node, create the following file at
...
12. Configure each node to accept your SSH keys as appropriate. Test your new
